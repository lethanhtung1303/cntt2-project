...
import {
  ...
  NormsLectureHoursService,
} from '../../../service/norms-lecture-hours.service';
...
export class AssignLectureHoursComponent {
  ...
  getLecturersByTrainingSys() {
    ...
    if (this.trainingSysForAssign == 1) {
      this.lecturers = this.universityLecturers
        .filter(
          (lecturer) =>
            lecturer.isTeachingPractice || lecturer.isTeachingTheory,
        )
        .map(({ id, fullName }) => ({ id, fullName }));
    }
    ...
    this.lecturers = this.lecturers
      .filter((lecturer) => {
        ...
        if (
          matchingNormsLecturer?.classificationLecturers ===
    ...
    if (this.trainingSysForAssign == 1) {
      ...
      if (subjectForAssign?.phanLoai === 'Lý thuyết') {
        this.lecturers = this.lecturers.filter((lecturer) => {
          ...
          return (
            !matchingNormsLecturer || matchingNormsLecturer.isTeachingTheory
          );
        });
      }
      ...
      if (subjectForAssign?.phanLoai === 'Thực hành') {
        this.lecturers = this.lecturers.filter((lecturer) => {
          ...
          return (
            !matchingNormsLecturer || matchingNormsLecturer.isTeachingPractice
          );
        });
      }
    }
  ...
  checkLecturerAssign() {
    ...
    return (
      lecturer &&
      lecturer.classification === 'Giảng viên cơ hữu' &&
      lecturer.status === 1
    );
  }
  ...
  onSubmit() {
    ...
    this.assignmentService.createAssignment(request).subscribe({
      next: () => {
        this.router
          .navigate([`/lecturers/${this.lecturerForAssign}`], {
  ...
  genAssignmentRequest(): AssignmentRequest {
