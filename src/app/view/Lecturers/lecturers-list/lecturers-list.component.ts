        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.lecturerService.deleteLecturer({
                    ...
                    next: () => {
                        this.messageService.add({key: idLecturer.toString(), severity: 'error', summary: 'Đã xoá', detail: `Bạn đã xoá giảng viên: ${idLecturer}`});
            ...
            reject: (type: ConfirmEventType) => {
                switch (type) {
                    case ConfirmEventType.REJECT:
                        this.messageService.add({key: idLecturer.toString(), severity: 'info', summary: 'Đã huỷ', detail: 'You have cancelled'});
                        break;
        ...
        this.confirmationService.confirm({
            ...
            accept: () => {
                ...
                this.lecturerService.deleteLecturer({
                    ...
                    next: () => {
                        this.messageService.add({key: 'deleteSelectedLecturers', severity: 'error', summary: 'Đã xoá', detail: 'Bạn đã xoá TẤT CẢ Giảng viên đã chọn'});
                        this.router.navigate([]).then(() => {
                            setTimeout(() => {
                                window.location.reload()
                            }, 1000);
            ...
            reject: (type: ConfirmEventType) => {
                switch (type) {
                    case ConfirmEventType.REJECT:
                        this.messageService.add({key: 'deleteSelectedLecturers', severity: 'info', summary: 'Đã huỷ', detail: 'You have cancelled'});
                        break;
                    case ConfirmEventType.CANCEL:
                        this.messageService.add({key: 'deleteSelectedLecturers', severity: 'info', summary: 'Đã huỷ', detail: 'You have cancelled'});
                        break;
